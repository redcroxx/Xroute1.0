...
<!--
    화면코드 : atomyBarcordPop
    화면명   :  애터미 상품 바코드 정보 등록
-->
<div id="atomyBarcordPop" class="pop_wrap">
    <!-- 그리드 시작 -->
...
<script type="text/javascript">
    // 등록 컬럼.
    var _GRIDCOLUMNS = [
            "krProductCode", "krProductName", "barcord"
    ];
    

    // 초기 로드.
    $(function() {
        $("#atomyBarcordPop").lingoPopup({
            title : "애터미 역직구 상품 바코드 등록",
            width : 1000,
            height : 800,
            buttons : {
                "저장" : {
    ...
    function setSave() {
        ...
        var url = "/fulfillment/atomy/barcord/pop/setSave.do";
        var sendData = {
            "paramList" : gridDataList
        };

        gfn_ajax(url, true, sendData, function(data, xhr) {
            if (data.CODE == "1") {
                cfn_msg("INFO", "애터미 역직구 상품 바코드 등록 완료");
                $("#atomyBarcordPop").lingoPopup("close", "OK");
            }
        });
    }
</script>