...
var jrfdir = "${DIR}/ubireport/work/";	// 리포트 파일 서버 경로
var jrffile = '${param.jrf}';											// 리포트파일명.
var args = '${param.arg}';	// 아규먼트
var print = '${param.print}';	// 프린터
var invcType = '${param.invcType}';	// 프린터
var dsname = "";				// 데이타소스명

var isdeflater = "false";	// 서버와의 통신 옵션 (ubigateway.property의 속성과 같아야함)
var isbase64 = "false";		// 서버와의 통신 옵션 (ubigateway.property의 속성과 같아야함)
var isunicode = "true";	// 서버와의 통신 옵션 (ubigateway.property의 속성과 같아야함)
...
function UbiPlugin_Init() {
	...
	if( Ubi_PluginCheck() ) {
		...
		document.write("   <param name='printname'           	value='" + print +"'>");
		document.write("   <param name='printsetmode'         	v2lue='1'>");
...
function ShowReport(ws) {
	...
	ws_viewer.setResize('hide');
	ws_viewer.SetVariable("printname", print); //프린터 설정
	ws_viewer.SetVariable("printsetmode",'1'); //프린터 설정
	ws_viewer.retrieve();
}
...
/* 출력 완료 후 호출. 플러그인. 웹소켓 */
  function PrintEnd(status) { 
	 if( runPlugin )
		alert("Print Status 2: " + document.getElementById("UbiViewer").getPrintStatus());
	else
		alert('Print Status 1: ' + status); 
		
	 var _args = args.split('#');
	var _invcType = invcType
	...
	//송장 출력횟수 증가
	  /*  if( _invcType === '0' ){
		...
		var compcdIndex = _args.indexOf('COMPCD');
		var orgcdIndex = _args.indexOf('ORGCD');
		var whcdIndex = _args.indexOf('WHCD');
		var reg_seq_Index = _args.indexOf('REG_SEQ');
		var relay_seq_s_Index = _args.indexOf('RELAY_SEQ_S');
		var relay_seq_e_Index = _args.indexOf('RELAY_SEQ_E');
		var wokeyIndex = _args.indexOf('WOKEY');
		
		(compcdIndex === -1)       ? compcdIndex = '1000'   : compcdIndex       = compcdIndex;
		(orgcdIndex === -1)        ? orgcdIndex = 'C0014'   : orgcdIndex        = orgcdIndex;
		(whcdIndex === -1)         ? whcdIndex = '2000'     : whcdIndex         = whcdIndex;
		(reg_seq_Index === -1)     ? reg_seq_Index = ''     : reg_seq_Index     = reg_seq_Index;
		(relay_seq_s_Index === -1) ? relay_seq_s_Index = '' : relay_seq_s_Index = relay_seq_s_Index;
		(relay_seq_e_Index === -1) ? relay_seq_e_Index = '' : relay_seq_e_Index = relay_seq_e_Index;
		(wokeyIndex === -1)        ? wokeyIndex = ''        : wokeyIndex	    = wokeyIndex;
		
		params.COMPCD      = _args[compcdIndex + 1];
		params.ORGCD       = _args[orgcdIndex + 1];
		params.WHCD        = _args[whcdIndex + 1];
		params.REG_SEQ     = _args[reg_seq_Index + 1];
		params.RELAY_SEQ_S = _args[relay_seq_s_Index + 1];
		params.RELAY_SEQ_E = _args[relay_seq_e_Index + 1];
		params.WOKEY       = _args[wokeyIndex + 1];
		
		url = '/fulfillment/stock/StockInsert/setInvcPrintNum.do';
		sendData = {'paramData':params};
		
	} else {
		var index = _args.indexOf('SEQ');
		var compcdIndex = _args.indexOf('COMPCD');
		var orgcdIndex = _args.indexOf('ORGCD');
		var whcdIndex = _args.indexOf('WHCD');
		var eachynIndex = _args.indexOf('EACH_YN');
		
		(index === -1)       ? index       = '0'  : index       = index;
		(compcdIndex === -1) ? compcdIndex = '1000'  : compcdIndex = compcdIndex;
		(orgcdIndex === -1)  ? orgcdIndex  = 'C0014' : orgcdIndex  = orgcdIndex;
		(whcdIndex === -1)   ? whcdIndex   = '4000'  : whcdIndex   = whcdIndex;
		(eachynIndex === -1) ? eachynIndex = 'Y'     : eachynIndex = eachynIndex;
		
		if(index !== -1 ){
			var _seq =_args[index+1].split(',');
			...
			if(!cfn_isEmpty(_seq)){
				...
				for(var i = 0; i < _seq.length; i++){
					...
					params.ORGCD   = _args[orgcdIndex + 1];
					params.WHCD    = _args[whcdIndex + 1];
					params.EACH_YN = _args[eachynIndex + 1];
