...
<!-- 
	화면코드 : P100350
	화면명    : 입고실적수량변경 - RealGrid 마스터 디테일 그리드 2개 패턴
-->
...
<div class="ct_bot_wrap">
	<div class="grid_top_wrap">
		...
		<div class="grid_top_right">
			<span style="color: red; font-size: 12px; font-weight: bold; padding-left:20px">※ 변경수량이 최종 입고실적수량입니다.</span>
		</div>
	</div>
	<div id="grid2"></div>
</div>
...
function grid1_Load() {
	...
	var columns = [
		...
		{name:'WISCHDT',header:{text:'입고요청일자'},width:90,formatter:'date',styles:{textAlignment:'center'}},
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {
		...
		gridView.onCurrentRowChanged = function(grid, oldRowIdx, newRowIdx) {
			...
			/*if (newRowIdx > -1) {--> 그냥 입고상태를 입고중으로 DISABLE 처리
				//입고상태가 300인 컬럼만 수량 변경처리
				var editable = $('#' + gid).gfn_getValue(newRowIdx, 'WISTS') === '300';
...
function grid2_Load() {
	...
	var columns = [
		...
		{name:'SEQ',header:{text:'지시순번'},width:70,dataType:'number',styles:{textAlignment:'far',numberFormat:'#,##0'}},
		{name:'WIKEY',header:{text:'입고번호'},width:150,styles:{textAlignment:'center'}},
		...
		{name:'AFTERQTY_EA',header:{text:'변경[EA]'},width:80,dataType:'number',styles:{textAlignment:'far',numberFormat:'#,##0'},
				footer:{styles:{textAlignment:'far',numberFormat:'#,##0'},text:'0',expression:'sum',groupExpression:'sum'}},
		{name:'AFTERQTY',header:{text:'변경수량'},width:100,dataType:'number',editable:true,styles:{textAlignment:'far',numberFormat:'#,##0'},editor:{maxLength:11,type:"number",positiveOnly:true,integerOnly:true},required:true,
			footer:{styles:{textAlignment:'far',numberFormat:'#,##0'},text:'0',expression:'sum',groupExpression:'sum'}},
			
		{name:'REMARK',header:{text:'비고'},width:300,editable:true},
		{name:'LOTKEY',header:{text:'로트키'},width:100,styles:{textAlignment:'center'}},
		{name:'LOT1',header:{text:cfn_getLotLabel(1)},width:100, editor:{maxLength:50}, formatter:cfn_getLotType(1), show:cfn_getLotVisible(1)},
		{name:'LOT2',header:{text:cfn_getLotLabel(2)},width:100, editor:{maxLength:50}, formatter:cfn_getLotType(2), show:cfn_getLotVisible(2)},
		{name:'LOT3',header:{text:cfn_getLotLabel(3)},width:100, editor:{maxLength:50}, formatter:cfn_getLotType(3), show:cfn_getLotVisible(3)},
...
function cfn_retrieve() {
	...
	var url = '/alexcloud/p100/p100350/getSearch.do';
...
function fn_getDetailList(param) {
	...
	var url = '/alexcloud/p100/p100350/getDetailList.do';
...
function cfn_execute(directmsg) {
	...
	if (flg) {
		...
		var url = '/alexcloud/p100/p100350/setExecute.do';
