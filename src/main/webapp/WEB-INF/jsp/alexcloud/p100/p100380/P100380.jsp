...
<div class="ct_top_wrap">
	<div id="tab1" class="ct_tab">
		<ul>
			<li><a href="#tab1Cont1">전표별</a></li>
			<li><a href="#tab1Cont2">품목별</a></li>
		</ul>
		<div id="tab1Cont1">
			<div class="ct_top_wrap">
				<div class="grid_top_wrap">
					<div class="grid_top_left">전표별 입고 리스트<span>(총 0건)</span></div>
		...
		<div id="tab1Cont2">
			<div class="grid_top_wrap">
				<div class="grid_top_left">품목별 입고 리스트<span>(총 0건)</span></div>
				<div class="grid_top_right"></div>
			</div>
			<div id="grid2"></div>
		</div>
...
$(function() {
	...
	$('#S_WIDT_FROM').val(cfn_getDateFormat(firstdate, 'yyyy-MM-dd'));
	$('#S_WIDT_TO').val(cfn_getDateFormat(lastdate, 'yyyy-MM-dd'));	
	//로그인 정보로 세팅
...
function grid1_Load() {
	...
	var columns = [
		...
		{name:'WISCHDT',header:{text:'입고요청일자'},width:110,formatter:'date',styles:{textAlignment:'center'},visible:false},
		...
		{name:'ORGCD',header:{text:'셀러코드'},width:100,styles:{textAlignment:'center'},visible:false},
		...
		{name:'WHNM',header:{text:'창고명'},width:80,styles:{textAlignment:'center'}},
		/* {name:'CUSTCD',header:{text:'거래처코드'},width:120,styles:{textAlignment:'center'}},
		{name:'CUSTNM',header:{text:'거래처명'},width:170}, */
		
		{name:'ITEMCD',header:{text:'상품코드'},width:100,styles:{textAlignment:'center'}},
		{name:'ITEM_NAME',header:{text:'상품명'},width:350},
		{name:'LOT1',header:{text:cfn_getLotLabel(1)},width:100, formatter:cfn_getLotType(1),styles:{textAlignment:'center'},width:120,show:cfn_getLotVisible(1)},
		{name:'LOT2',header:{text:cfn_getLotLabel(2)},width:100, formatter:cfn_getLotType(2),width:120,show:cfn_getLotVisible(2),styles:{textAlignment:'center'}},
		{name:'LOT3',header:{text:cfn_getLotLabel(3)},width:100, formatter:cfn_getLotType(3),width:120,show:cfn_getLotVisible(3),styles:{textAlignment:'center'}},
		{name:'LOT4',header:{text:cfn_getLotLabel(4)},width:100, formatter:'combo',styles:{textAlignment:'center'},width:120,show:cfn_getLotVisible(4),comboValue:gCodeLOT4},
		{name:'LOT5',header:{text:cfn_getLotLabel(5)},width:100, formatter:'combo',styles:{textAlignment:'center'},width:120,show:cfn_getLotVisible(5),comboValue:gCodeLOT5},
		
		{name:'INBOXQTY',header:{text:'박스입수량'},width:100,dataType:'number',styles:{textAlignment:'far',numberFormat:'#,##0'}},
		{name:'WIQTY_BOX',header:{text:'입고수량(BOX)'},width:100,dataType:'number',styles:{textAlignment:'far',numberFormat:'#,##0'},
			footer:{styles:{textAlignment:'far',numberFormat:'#,##0'},text:'0',expression:'sum',groupExpression:'sum'}},
		{name:'WIQTY_EA',header:{text:'입고수량(EA)'},width:100,dataType:'number',styles:{textAlignment:'far',numberFormat:'#,##0'},
			footer:{styles:{textAlignment:'far',numberFormat:'#,##0'},text:'0',expression:'sum',groupExpression:'sum'}},
		...
		{name:'COMPNM',header:{text:'회사명'},width:120,show:false},
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {
		...
		$('#'+gid).gridView().setColumnProperty(
			gridView.columnByField("LOT1"),
			"dynamicStyles", [{
				criteria: ["values['LOT1'] <> ''"]
				, styles: ["background=#ffff1a"]
			}]
		)
		
		$('#'+gid).gridView().setColumnProperty(
			gridView.columnByField("LOT2"),
			"dynamicStyles", [{
				criteria: ["values['LOT2'] <> ''"]
				, styles: ["background=#ffff1a"]
			}]
		)
		
		$('#'+gid).gridView().setColumnProperty(
			gridView.columnByField("LOT3"),
			"dynamicStyles", [{
				criteria: ["values['LOT3'] <> ''"]
				, styles: ["background=#ffff1a"]
			}]
		)
		
		$('#'+gid).gridView().setColumnProperty(
			gridView.columnByField("WIQTY"),
			"dynamicStyles", [{
				criteria: ["values['WIQTY'] <> ''"]
				, styles: ["background=#ffff1a"]
			}]
		)
...
function cfn_retrieve() {
	...
	var url = '/alexcloud/p100/p100380/getSearch.do';
