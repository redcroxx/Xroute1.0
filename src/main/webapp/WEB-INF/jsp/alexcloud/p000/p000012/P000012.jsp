...
<div class="ct_right_wrap">
	<div class="grid_top_wrap">
		<div class="grid_top_left">기간별 매입단가</div>
		<div class="grid_top_right">
			<input type="button" id="btn_upload" class="cmb_normal" value="업로드" onclick="pfn_upload('P000012');" />
...
function grid2_Load() {
	...
	var columns = [
		...
		{name:'UNITCOST',header:{text:'매입단가'},required:true,width:100,dataType:'number',styles:{textAlignment:'far',numberFormat:'#,##0'},minVal:0,
	...
	$grid.gfn_setGridEvent(function(gridView, dataProvider) {
		...
		var newDynamicStyles = [{
			criteria: "(value['ISTODAY'] = 'T')",
			styles: {background:'#b6dd61'}
			},
		];
		...
		gridView.onCurrentRowChanged = function(grid, oldRowIdx, newRowIdx) {
			if (newRowIdx > -1) {
				...
				if(state === 'created') {
					$grid.gfn_setColDisabled(['STARTDT', 'ENDDT','UNITCOST'],true);
				}
				else if(rowData.ENDDT === '99991231'){
					$grid.gfn_setColDisabled(['STARTDT', 'ENDDT'],false);
					$grid.gfn_setColDisabled(['UNITCOST'],true);
				}
				else if(rowData.ENDDT > cfn_getDateFormat(today,'yyyyMMdd')) {
					$grid.gfn_setColDisabled(['ENDDT','UNITCOST'],true);
					$grid.gfn_setColDisabled(['STARTDT'],false);
				}
				else {
					$grid.gfn_setColDisabled(['STARTDT','ENDDT','UNITCOST'],false);
				}
			} 
		};
	});	
...
function cfn_retrieve() {
	...
	var url = '/alexcloud/p000/p000012/getSearch.do';
...
function cfn_save(directmsg) {
	...
	if (flg) {
		var sid = 'sid_setSave';
		var url = '/alexcloud/p000/p000012/setSave.do';
...
function fn_getDetailList(param) {
	...
	var url = '/alexcloud/p000/p000012/getDetailList.do';
...
function fn_detailDel(){
	...
	if(state === 'created'){
		...
		cfn_msg('WARNING', '기등록 내역은 삭제불가합니다.');
	}	
}
</script>
