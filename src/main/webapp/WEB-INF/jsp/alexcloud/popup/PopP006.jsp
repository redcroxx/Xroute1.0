...
<!-- 
	...
	화면명    : 팝업-품목코드
-->
<div id="popP006" class="pop_wrap">
	<!-- 검색조건영역 시작 -->
	<div id="ct_sech_wrap">
		<form id="frm_search_popP006" action="#" onsubmit="return false">
			...
			<button type="button" class="cmb_pop_search" onclick="fn_search_popP006()"></button>
...
var columns = [
	...
	{name:'NAME',header:{text:'품목명'},width:250},
...
$(function() {
	$('#popP006').lingoPopup({
		...
		buttons: {
			'확인': {
				...
				click: function() {
					...
					if(tabidx === 1) {
						var checkRows = $('#grid_popP006A').gridView().getCheckedRows();
						var gridData = $('#grid_popP006A').gfn_getDataRows(checkRows);
						...
						var gridData = $('#grid_popP006C').gfn_getDataList();
		...
		open: function(data) {
			cfn_bindFormData('frm_search_popP006', data);
			...
			if (gridData.length > 0) {
				$('#grid_popP006A').gfn_setDataList(data.gridData);
			} else {
				fn_search_popP006();
			}
		}
...
function grid_Load_popP006() {
	var gid = 'grid_popP006';
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {		
		...
		gridView.onDataCellDblClicked = function (grid, index) {
			var rowidx = $('#grid_popP006').gfn_getCurRowIdx();
			var gridData = $('#grid_popP006').gfn_getDataRow(rowidx);
			$('#grid_popP006C').gfn_addRow(gridData);
		};
		
	});
}

function grid_Load_popP006C() {
	var gid = 'grid_popP006C';
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {		
		//셀 더블클릭
		gridView.onDataCellDblClicked = function (grid, index) {
			var rowidx = $('#grid_popP006C').gfn_getCurRowIdx();
			$('#grid_popP006C').gfn_delRow(rowidx);
		};
...
function grid_Load_popP006A() {
	var gid = 'grid_popP006A';
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {		
		//셀 더블클릭
		gridView.onDataCellDblClicked = function (grid, index) {
			var rowidx = $('#grid_popP006A').gfn_getCurRowIdx();
			var gridData = [$('#grid_popP006A').gfn_getDataRow(rowidx)];
			$('#popP006').lingoPopup('setData', gridData);
			$('#popP006').lingoPopup('close', 'OK');
		};
...
/* 검색 */
function fn_search_popP006() {
	var paramData = cfn_getFormData('frm_search_popP006');
	var sendData = {'paramData':paramData};
	var url = '/alexcloud/popup/popP006/search.do';
	...
	gfn_ajax(url, true, sendData, function(data, xhr) {
		...
		if(tabidx === 1) {
			$('#grid_popP006A').gfn_setDataList(data.resultList);
		} else if(tabidx === 2) {
			$('#grid_popP006').gfn_setDataList(data.resultList);
		}
	});
}
...
function fn_grid1RowAdd(){
	var checkRows = $('#grid_popP006').gridView().getCheckedRows();
	var gridData = $('#grid_popP006').gfn_getDataRows(checkRows);
	...
	for (var i=0, len=checkRows.length; i<len; i++) {
		$('#grid_popP006C').gfn_addRow(gridData[i]);
	}
	//체크해제
	$('#grid_popP006').gridView().checkAll(false);
}

//선택품목에서 검색결과로 자리 옮기기
function fn_grid1RowDel(){
	var checkRows = $('#grid_popP006C').gridView().getCheckedRows();
	...
	for (var i=checkRows.length - 1; i>=0; i--) {
		$('#grid_popP006C').gfn_delRow(checkRows[i]);
	}
}
</script>