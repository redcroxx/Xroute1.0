...
<!-- 
	...
	화면명    : 업로드 공통팝업
-->
<div id="popEP200363" class="pop_wrap">
	<!-- 검색조건영역 시작 -->
	<div id="ct_sech_wrap">
		<form id="frm_search_popEP200363" action="#" onsubmit="return false">
			<p id="lbl_name">업로드</p>
			...
			<ul class="sech_ul">
				<li class="sech_li">	
					<div>
						<textarea id="popEP200363_REMARK" name="popEP200363_REMARK" class="cmc_area disabled" readonly="readonly" cols="45" style="width:800px;height:50px;"></textarea>
					</div>
				</li>
			</ul>
		</form>
...
$(function() {
	$('#popEP200363').lingoPopup({
		...
		buttons: {
			'체크': {
				...
				click: function() {
					...
					$('#grid_popEP200363').gridView().commit(true);
					...
					// 메시지 초기화
					ckmsg = [];
					var columns = $('#grid_popEP200363').gridView().getColumns();
					var gridDataRows = $('#grid_popEP200363').gfn_getDataList();
					for (var i=0, len=gridDataRows.length; i<len; i++) {
						$('#grid_popEP200363').gfn_setValue(i, 'CHKMSG', "");
					}
					
					// 자릿수 체크
					var failData = $('#grid_popEP200363').gridView().checkValidateCells();
					$('#grid_popEP200363').gridView().clearInvalidCellList();
					...
					for (var i=0, len=ckmsg.length; i<len; i++) {
						...
						if(!cfn_isEmpty(ckmsg[i])){
							$('#grid_popEP200363').gfn_setValue(j, 'CHKMSG', $('#grid_popEP200363').gfn_getValue(j, 'CHKMSG') + ckmsg[i].message + "," );
						}						
					}
					...
					var gridDataList = $('#grid_popEP200363').gfn_getDataList(true, false);
					...
					var whcd = $('#S_WHCD').val();
					var url = '/alexcloud/popup/popEP200363/setCheck.do';
					...
					gfn_ajax(url, true, sendData, function(data, xhr) {
						...
						$('#grid_popEP200363').gfn_setDataList(gridData);
						
						// 체크 확인
						chk = 1;
						for (var i=0, len=gridDataList.length; i<len; i++) {
							if(!cfn_isEmpty($('#grid_popEP200363').gfn_getValue(i, 'CHKMSG'))){
								chk = 0;
			...
			'업로드': {
				text: '업로드',
				click: function() {
					var gridDataList = $('#grid_popEP200363').gfn_getDataList();
		...
		open: function(data) {
			...
			$("#xlf").bind("change", function(e){gfn_handleXlsFile(e,'grid_popEP200363', gcolumns)});
			
			cfn_bindFormData('frm_search_popEP200363', data);			
...
function fn_popEP200363close(){
	$('#popEP200363').lingoPopup('setData', '');
	$('#popEP200363').lingoPopup('close', 'OK');
}

var ckmsg = [];
var gcolumns = [];
// 그리드 생성
function grid_Load_popEP200363(gridData) {
	var gid = 'grid_popEP200363';
	var columns = [];
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {
		...
		gridView.onValidateColumn = function(grid, column, inserting, value) {
			...
			var gCols = $('#grid_popEP200363').columns();
...
function fn_uploadAddRow() {
	...
	$('#grid_popEP200363').gfn_addRow();
}

//그리드 행삭제 버튼 클릭
function fn_uploadDelRow() {
	var $grid1 = $('#grid_popEP200363');
...
/* 업로드 화면에 따른 그리드 정보 조회 */
function fn_init_popEP200363(){
	var paramData = cfn_getFormData('frm_search_popEP200363');
	var sendData = {'paramData':paramData};
	var url = '/alexcloud/popup/popEP200363/init.do';
	
	gfn_ajax(url, true, sendData, function(data, xhr) {
		$('#lbl_name').text(data.mstData.PGMNM);
		$('#popEP200363_REMARK').val(data.mstData.REMARK.replace(/\\r\\n/g, '\r\n'));
		grid_Load_popEP200363(data.resultList);
