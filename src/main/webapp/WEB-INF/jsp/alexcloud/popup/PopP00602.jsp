...
<!-- 
	...
	화면명    : 팝업-품목코드(거래처지정)
-->
<div id="popP00602" class="pop_wrap">
	<!-- 검색조건영역 시작 -->
	<div id="ct_sech_wrap">
		<form id="frm_search_popP00602" action="#" onsubmit="return false">
			...
			<button type="button" class="cmb_pop_search" onclick="fn_search_popP00602()"></button>
...
var columns = [
	...
	{name:'NAME',header:{text:'품목명'},width:240},
	...
	{name:'ITEMTYPE',header:{text:'품목유형'},width:100,styles:{textAlignment:'center'},formatter:'combo',comboValue:'${gCodeITEMTYPE}'},
	...
	{name:'ITEMCAT1NM',header:{text:'대분류'},width:100,styles:{textAlignment:'center'}},
	{name:'ITEMCAT2NM',header:{text:'중분류'},width:100,styles:{textAlignment:'center'}},
	{name:'ITEMCAT3NM',header:{text:'소분류'},width:100,styles:{textAlignment:'center'}},
	{name:'SETYN',header:{text:'세트여부'},width:100,styles:{textAlignment:'center'},formatter:'combo',comboValue:'${gCodeSETYN}'},
	{name:'LENGTH',header:{text:'깊이'},width:100,styles:{textAlignment:'center'}},
	{name:'WIDTH',header:{text:'폭'},width:100,styles:{textAlignment:'center'}},
	{name:'HEIGHT',header:{text:'높이'},width:100,styles:{textAlignment:'center'}},
	{name:'CAPACITY',header:{text:'용량'},width:100,styles:{textAlignment:'center'}},
	{name:'MANUFACTURE',header:{text:'제조사'},width:100,styles:{textAlignment:'center'}},
	{name:'MANUCOUNTRY',header:{text:'제조국'},width:100,styles:{textAlignment:'center'}},
	{name:'CUSTNM',header:{text:'거래처(지정)'},width:100,styles:{textAlignment:'center'}},
...
$(function() {
	$('#popP00602').lingoPopup({
		...
		buttons: {
			'확인': {
				...
				click: function() {
					...
					if(tabidx === 1) {
						var checkRows = $('#grid_popP00602A').gridView().getCheckedRows();
						var gridData = $('#grid_popP00602A').gfn_getDataRows(checkRows);
						...
						var gridData = $('#grid_popP00602C').gfn_getDataList();
		...
		open: function(data) {
			cfn_bindFormData('frm_search_popP00602', data);
			...
			grid_Load_popP00602();
			grid_Load_popP00602C();
			grid_Load_popP00602A();

			var gridData = data.gridData;
			                         
			if (gridData.length > 0) {
				$('#grid_popP00602A').gfn_setDataList(data.gridData);
			} else {
				fn_search_popP00602();
			}
		}
...
function grid_Load_popP00602() {
	var gid = 'grid_popP00602';
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {		
		...
		gridView.onDataCellDblClicked = function (grid, index) {
			var rowidx = $('#grid_popP00602').gfn_getCurRowIdx();
			var gridData = $('#grid_popP00602').gfn_getDataRow(rowidx);
			$('#grid_popP00602C').gfn_addRow(gridData);
		};
		
	});
}

function grid_Load_popP00602C() {
	var gid = 'grid_popP00602C';
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {		
		//셀 더블클릭
		gridView.onDataCellDblClicked = function (grid, index) {
			var rowidx = $('#grid_popP00602C').gfn_getCurRowIdx();
			$('#grid_popP00602C').gfn_delRow(rowidx);
		};
...
function grid_Load_popP00602A() {
	var gid = 'grid_popP00602A';
	...
	$('#' + gid).gfn_setGridEvent(function(gridView, dataProvider) {		
		//셀 더블클릭
		gridView.onDataCellDblClicked = function (grid, index) {
			var rowidx = $('#grid_popP00602A').gfn_getCurRowIdx();
			var gridData = [$('#grid_popP00602A').gfn_getDataRow(rowidx)];
			$('#popP00602').lingoPopup('setData', gridData);
			$('#popP00602').lingoPopup('close', 'OK');
		};
...
/* 검색 */
function fn_search_popP00602() {
	var paramData = cfn_getFormData('frm_search_popP00602');
	var sendData = {'paramData':paramData};
	var url = '/alexcloud/popup/popP00602/search.do';
	...
	gfn_ajax(url, true, sendData, function(data, xhr) {
		...
		if(tabidx === 1) {
			$('#grid_popP00602A').gfn_setDataList(data.resultList);
		} else if(tabidx === 2) {
			$('#grid_popP00602').gfn_setDataList(data.resultList);
		}
	});
}
...
function fn_grid1RowAdd(){
	var checkRows = $('#grid_popP00602').gridView().getCheckedRows();
	var gridData = $('#grid_popP00602').gfn_getDataRows(checkRows);
	...
	for (var i=0, len=checkRows.length; i<len; i++) {
		$('#grid_popP00602C').gfn_addRow(gridData[i]);
	}
	//체크해제
	$('#grid_popP00602').gridView().checkAll(false);
}

//선택품목에서 검색결과로 자리 옮기기
function fn_grid1RowDel(){
	var checkRows = $('#grid_popP00602C').gridView().getCheckedRows();
	...
	for (var i=checkRows.length - 1; i>=0; i--) {
		$('#grid_popP00602C').gfn_delRow(checkRows[i]);
	}
}
</script>