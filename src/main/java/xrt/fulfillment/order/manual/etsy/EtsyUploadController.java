 ...
 */
@Controller
@RequestMapping(value = "/fulfillment/order/etsy")
public class EtsyUploadController {

	@Resource
	private CommonBiz commonBiz;
	@Resource
	private EtsyUploadBiz biz;

	Logger logger = LoggerFactory.getLogger(EtsyUploadController.class);
	...
	public String view(ModelMap model) throws Exception {
		...
		return "fulfillment/order/upload/EtsyUpload";
	}
	...
	public LRespData upload(@RequestBody LReqData reqData) throws Exception {
		...
		LDataMap validMap = biz.valid(paramMap, dataList);
		String code = validMap.getString("code");
		if (!code.equals("200")) {
			throw new LingoException("엣시 오더 파일 업로드시 \n 오류가 발생 하였습니다.");
		}
		Map<String, Object> commMap = (Map<String, Object>) validMap.get("commMap");
		List<EtsyVo> orderList = (List<EtsyVo>) validMap.get("orderList");
		logger.debug("2. 엣시 주문 정보 저장");
		biz.orderInsert(orderList);
		logger.debug("3. 엣시 아마존 주문 정보 조회");

		logger.debug("4. Xroute Torder, TorderDtl 설정");
		LDataMap procgMap= biz.processing(commMap, orderList);
