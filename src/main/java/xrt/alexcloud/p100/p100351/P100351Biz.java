 ...
 */
@Service
public class P100351Biz extends DefaultBiz {
	//검색
	public List<LDataMap> getSearch(LDataMap param) throws Exception {
		return dao.select("P100351Mapper.getSearch", param);
	}

	//디테일 리스트 가져오기
	public List<LDataMap> getDetailList(LDataMap param) throws Exception {
		return dao.select("P100351Mapper.getDetailList", param);
	}
	...
	public LDataMap setExecute(List<LDataMap> paramList) throws Exception {
		...
		for (int i = 0; i < paramList.size(); i++) {
			...
			dao.update("P100351Mapper.setExecute", executeData);
