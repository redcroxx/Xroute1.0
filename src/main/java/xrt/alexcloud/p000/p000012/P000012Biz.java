...
@Service
public class P000012Biz extends DefaultBiz {

	//검색
	public List<LDataMap> getSearch(LDataMap param) throws Exception {
		return dao.select("P000012Mapper.getSearch", param);
	}

	//디테일 리스트 가져오기
	public List<LDataMap> getDetailList(LDataMap param) throws Exception {
		return dao.select("P000012Mapper.getDetailList", param);
	}
	//저장
	public LDataMap setSave(List<LDataMap> paramList) throws Exception {
		...
		for(int i = 0; i<paramList.size(); i++) {
			...
			dao.select("P000012Mapper.updateCost", detailData);
