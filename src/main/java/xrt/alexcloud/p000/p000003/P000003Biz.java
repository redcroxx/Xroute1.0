 ...
 */
@Service
public class P000003Biz extends DefaultBiz {
	//검색
	public List<LDataMap> getSearch(LDataMap param) throws Exception {
		return dao.select("P000003Mapper.getSearch", param);
	}
	...
	public LDataMap setSave(List<LDataMap> paramList) throws Exception {
		...
		for (int i=0; i<paramListSize; i++) {
			...
			} else if (Util.isEmpty(map.get("CUSTCD"))) {
				throw new LingoException("처리 중 오류가 발생하였습니다.\n거래처코드는 필수 입력값 입니다.");
			} else if (Util.isEmpty(map.get("NAME"))) {
				throw new LingoException("처리 중 오류가 발생하였습니다.\n거래처명은 필수 입력값 입니다.");
			} else if ("N".equals(map.get("ISSUPPLIER")) && "N".equals(map.get("ISSHIPPER")) && "N".equals(map.get("ISDELIVERY"))) {
				throw new LingoException("처리 중 오류가 발생하였습니다.\n공급처, 납품처, 배송처 중에서\n1가지는 필수로 체크해야 합니다.");
			}

			LDataMap detailData = (LDataMap) dao.selectOne("P000003Mapper.getDetail", map);

			if ("I".equals(map.get("IDU"))) {
				if (detailData != null && !Util.isEmpty(detailData.get("CUSTCD"))) {
					throw new LingoException("처리 중 오류가 발생하였습니다.\n거래처코드[" + map.get("CUSTCD") + "]는 이미 존재하는 거래처코드 입니다.");
				}

				scnt = dao.insert("P000003Mapper.insertMst", map);
			} else if ("U".equals(map.get("IDU"))) {
				if (detailData == null || Util.isEmpty(detailData.get("CUSTCD"))) {
					throw new LingoException("처리 중 오류가 발생하였습니다.\n거래처코드[" + map.get("CUSTCD") + "]는 존재하지 않는 거래처입니다.");
				} else if ("N".equals(detailData.get("ISUSING"))) {
					throw new LingoException("처리 중 오류가 발생하였습니다.\n거래처코드[" + map.get("CUSTCD") + "]는 사용중지 되었습니다.");
				}

				scnt = dao.update("P000003Mapper.updateMst", map);
			}
		}
	...
	public LDataMap setDelete(LDataMap param) throws Exception {
		...
		if (Util.isEmpty(param.get("COMPCD"))) {
			throw new LingoException("처리 중 오류가 발생하였습니다.\n회사코드가 없습니다.");
		} else if (Util.isEmpty(param.get("CUSTCD"))) {
			throw new LingoException("처리 중 오류가 발생하였습니다.\n거래처코드가 없습니다.");
		}
		...
		scnt = dao.update("P000003Mapper.updateIsusing", param);
