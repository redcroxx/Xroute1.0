/**
 * 1. Project Name : SEEDSample
 * 2. Package : kr.re.etri.security
 * 3. File Name : SEED128.java
 * 4. Author : Jemin Song
 * 5. Create Date : 2011. 12. 12.
 */
package xrt.lingoframework.utils;  // 고객님 경로로 수정
...
/************************** SEED encrtyption function *************************/
		...
		byte outData[])				// [out] encrypted data
	{
		...
		L0[0] = (pbData[0]&0x000000ff);
		L0[0] = ((L0[0])<<8)^(pbData[1]&0x000000ff);
		L0[0] = ((L0[0])<<8)^(pbData[2]&0x000000ff);
		L0[0] = ((L0[0])<<8)^(pbData[3]&0x000000ff);

		L1[0] = (pbData[4]&0x000000ff);
		L1[0] = ((L1[0])<<8)^(pbData[5]&0x000000ff);
		L1[0] = ((L1[0])<<8)^(pbData[6]&0x000000ff);
		L1[0] = ((L1[0])<<8)^(pbData[7]&0x000000ff);

		R0[0] = (pbData[8]&0x000000ff);
		R0[0] = ((R0[0])<<8)^(pbData[9]&0x000000ff);
		R0[0] = ((R0[0])<<8)^(pbData[10]&0x000000ff);
		R0[0] = ((R0[0])<<8)^(pbData[11]&0x000000ff);

		R1[0] = (pbData[12]&0x000000ff);
		R1[0] = ((R1[0])<<8)^(pbData[13]&0x000000ff);
		R1[0] = ((R1[0])<<8)^(pbData[14]&0x000000ff);
		R1[0] = ((R1[0])<<8)^(pbData[15]&0x000000ff);
		...
		byte pbUserKey[])		// [in]	secret user key
	{
		...
		A[0] = (pbUserKey[0]&0x000000ff);
		A[0] = (A[0]<<8)^(pbUserKey[1]&0x000000ff);
		A[0] = (A[0]<<8)^(pbUserKey[2]&0x000000ff);
		A[0] = (A[0]<<8)^(pbUserKey[3]&0x000000ff);

		B[0] = (pbUserKey[4]&0x000000ff);
		B[0] = (B[0]<<8)^(pbUserKey[5]&0x000000ff);
		B[0] = (B[0]<<8)^(pbUserKey[6]&0x000000ff);
		B[0] = (B[0]<<8)^(pbUserKey[7]&0x000000ff);

		C[0] = (pbUserKey[8]&0x000000ff);
		C[0] = (C[0]<<8)^(pbUserKey[9]&0x000000ff);
		C[0] = (C[0]<<8)^(pbUserKey[10]&0x000000ff);
		C[0] = (C[0]<<8)^(pbUserKey[11]&0x000000ff);

		D[0] = (pbUserKey[12]&0x000000ff);
		D[0] = (D[0]<<8)^(pbUserKey[13]&0x000000ff);
		D[0] = (D[0]<<8)^(pbUserKey[14]&0x000000ff);
		D[0] = (D[0]<<8)^(pbUserKey[15]&0x000000ff);
