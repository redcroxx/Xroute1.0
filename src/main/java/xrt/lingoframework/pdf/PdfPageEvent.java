package xrt.lingoframework.pdf;
...
public class PdfPageEvent extends PdfPageEventHelper {
	...
	public PdfPageEvent(HashMap<String, Object> param, HttpServletRequest req) {
		...
		try {
			...
			title = Util.NVL(param.get("title"));
	...
	public StringBuilder getHtmlContent() {
		...
		htmlStr.append("	body,div,table,td {font:10pt NanumGothic;color:#2A2A2A;}");
		...
		for (int i=0; i<colModelLength; i++) {
			htmlStr.append("<th>");
			htmlStr.append(Util.NVL(colModelList.get(i).get("label")));
			htmlStr.append("</th>");
    	}
		...
		for (int i=0; i<gridDataLength; i++) {
			...
			for (int j=0; j<colModelLength; j++) {
				...
				if ("integer".equals(colModelList.get(j).get("formatter"))) {
					htmlStr.append(String.format("%,d", Integer.parseInt(Util.NVL(gridDataList.get(i).get(colModelList.get(j).get("name")), "0"))));
				} else if ("number".equals(colModelList.get(j).get("formatter"))) {
					htmlStr.append(String.format("%,f", Double.parseDouble(Util.NVL(gridDataList.get(i).get(colModelList.get(j).get("name")), "0"))));
				} else {
					htmlStr.append(Util.NVL(gridDataList.get(i).get(colModelList.get(j).get("name"))));
				}
